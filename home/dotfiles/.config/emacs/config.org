...
(use-package org
  ...
  (org-babel-do-load-languages 'org-babel-load-languages '((mermaid . t) (shell . t))))
#+end_src
...
(use-package ob-mermaid
  ...
  (org-babel-default-header-args:mermaid
   '((:results . "file")
     (:exports . "results")
     (:puppeteer-config-file . "/home/ben/.config/puppeteer.json")
     (:mermaid-config-file . "/home/ben/.config/mermaid.json")))
  (ob-mermaid-cli-path "/home/ben/.pnpm/mmdc"))
#+end_src
...
(defun my-org-agenda-add-motd ()
  "Add a MOTD to the top-right of the org-agenda buffer."
  (let ((motd "ℹ️ MOTD: eat less, move more   ")
        (width (window-width)))
    (save-excursion
      (goto-char (point-min))
      (let ((motd-line (concat (make-string (- width (length motd)) ? ) motd)))
        (insert (propertize motd-line 'face 'font-lock-comment-face) "\n")))))

;; Hook the function to the org-agenda finalize process
(add-hook 'org-agenda-finalize-hook 'my-org-agenda-add-motd)
#+end_src
** ~org-tempo~
#+begin_src emacs-lisp
(require 'org-tempo)
...
#+end_src
* Editing & Programming
update project list when switching project
#+begin_src emacs-lisp
(defun my/before-project-switch-project (&optional dir)
  ;; (interactive)
  (project-forget-zombie-projects)
  (project-remember-projects-under "~/src"))
(advice-add 'project-switch-project :before #'my/before-project-switch-project)
#+end_src
...
(use-package aidermacs
  ...
  (aidermacs-architect-model "openrouter/anthropic/claude-3.5-sonnet")
...
(use-package doom-themes
  ...
  (custom-set-faces
   `(org-scheduled-today ((t (:foreground ,(doom-color 'fg)))))
