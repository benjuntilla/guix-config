...
see [[https://github.com/emacs-eaf/emacs-application-framework/discussions/799][discussion]] and related [[https://github.com/emacs-eaf/emacs-application-framework/pull/866][pr]]
#+begin_src emacs-lisp :tangle no
(use-package eaf
...
(use-package org-roam-bibtex
  :after org-roam
  :custom (orb-note-actions-interface 'hydra)
  :general (org-mode-map
            "C-c l n" 'orb-insert-link)
  :hook (org-roam-mode))
#+end_src
*** ~org-ref~
#+begin_src emacs-lisp
(use-package org-ref
  ...
  :demand t
  :general (org-mode-map
            "C-c l h" 'org-ref-insert-link-hydra/body)
  :custom
...
(use-package org-cliplink
  :general (org-mode-map "C-c l p" 'my/org-cliplink-without-domain-name))
#+end_src
** ~org-super-agenda~
#+begin_src emacs-lisp
...
#+end_src
** ~dired~
Disable details by default
#+begin_src emacs-lisp
(add-hook 'dired-mode-hook #'dired-hide-details-mode)
#+end_src

Set image viewer for dired
#+begin_src emacs-lisp
...
Install ~dired-narrow~ for match filtering
#+begin_src emacs-lisp
...
Install ~dired-collapse~ to automatically collapse chains of single-child dirs
#+begin_src emacs-lisp
(use-package dired-collapse
  :hook dired-mode)
#+end_src

Install ~dired-filter~ for advanced, stackable filtering
#+begin_src emacs-lisp
(use-package dired-filter
  ...
  (dired-filter-group "default")
  :hook (dired-mode . dired-filter-group-mode))
#+end_src

Install ~dired-avfs~ to browse archives w/ ~avfs~
#+begin_src emacs-lisp
(use-package dired-avfs)
#+end_src

Install ~dired-open~ to open files w/ external applications
#+begin_src emacs-lisp
...
Install ~diredfl~ for pretty colorful font lock rules
#+begin_src emacs-lisp
(use-package diredfl
  :config (diredfl-global-mode))
#+end_src

Install ~dired-subtrees~ for more intuitive viewing of subdirectories
#+begin_src emacs-lisp
(use-package dired-subtree)
#+end_src

Install ~dired-ranger~ for intuitive copy & move/paste via clipboard
#+begin_src emacs-lisp
(use-package dired-ranger
  :general ('normal 'dired-mode-map
            ...
            "P" #'dired-ranger-move))
#+end_src
** filetags integration into dired
#+begin_src emacs-lisp
...
(use-package projectile
  :delight
  :general ("C-c p" '(:keymap projectile-command-map :package projectile))
...
Use ~pyenv~ to handle Python virtual environments
#+begin_src emacs-lisp
(use-package pyenv)
#+end_src
...
(use-package tex-site
  :straight auctex  ;; equivalent to :ensure auctex
  :custom
  (TeX-auto-save t)
  (TeX-parse-self t)
  ...
  :general (LaTeX-mode-map
            "TAB" 'cdlatex-tab)
  :hook
  (LaTeX-mode . (lambda () (reftex-mode) (olivetti-mode) (display-line-numbers-mode) (TeX-source-correlate-mode) (outline-minor-mode))))
#+end_src
*** ~xenops~
#+begin_src emacs-lisp :tangle no
(use-package xenops
...
(use-package org-journal
    ...
    :general ("C-c j" 'org-journal-new-entry))
#+end_src
** ~smart-tabs-mode~
#+begin_src emacs-lisp
...
(use-package smart-tabs-mode
  ...
  (smart-tabs-insinuate 'lua 'javascript))
#+end_src
** ~literate-calc-mode~
#+begin_src emacs-lisp
...
(use-package emacs-conflict
  :straight (emacs-conflict :host github :repo "ibizaman/emacs-conflict"))
#+end_src
** ~smartparens~
#+begin_src emacs-lisp
...
#+end_src
*** ~vimish-fold~ and ~evil-vimish-fold~
#+begin_src emacs-lisp
...
(use-package flyspell-correct
  ...
  (flyspell-mouse-map "<mouse-1>" 'flyspell-correct-wrapper))
#+end_src
** Snippets
*** ~yasnippet~
#+begin_src emacs-lisp
...
#+end_src
*** Tweak auto window splits & source-file-buffer switching
[[https://github.com/nex3/perspective-el#some-musings-on-emacs-window-layouts][Source]]
#+begin_src emacs-lisp :tangle no
...
#+end_src
*** TODO ~perspective~
Isolates & manages different sets of buffers & window configurations associated with different working contexts
#+begin_src emacs-lisp :tangle no
(use-package perspective
  :general (("C-x b" 'persp-switch-to-buffer*)
            ("C-x k" 'persp-kill-buffer*)
            ("C-x C-b" 'persp-list-buffers))
  :hook (kill-emacs . persp-state-save)
  ;; :custom
  ;; (persp-state-default-file "~/.emacs.d/test")
  :config
  (persp-mode))
#+end_src
** smartscan
...
Use ~adaptive-wrap~ to indent paragraphs
#+begin_src emacs-lisp
...
the following package prevents emacs daemons from connecting to clients
#+begin_src emacs-lisp :tangle no
(use-package highlight-indent-guides
  :delight
  :hook (prog-mode org-mode)
  :custom (highlight-indent-guides-responsive 'top)
...
(use-package volatile-highlights
  ...
  :straight '(volatile-highlights :host github :repo "k-talo/volatile-highlights.el"))
#+end_src
...
#+begin_src emacs-lisp
  (use-package corfu
    :custom
    ;; (tab-always-indent 'complete)  ;; indentation+completion w/ TAB
    (completion-cycle-threshold 3)  ;; cycle w/ TAB when under 3 candidates
    (corfu-cycle t)
    (corfu-auto t)
    :config
    (global-corfu-mode))
...
(use-package citeproc-org
  :custom (citeproc-org-ignore-backends nil "references will always be rendered with citeproc-el, e.g., w/ latex or beamer")
...
#+end_src
* irc
Create function to connect to libera.chat
#+begin_src emacs-lisp
(defun my/libera()
  (interactive)
...
Set prompt to show channel name
#+begin_src emacs-lisp
(setq erc-prompt  (lambda () (concat (buffer-name) " > ")))
#+end_src

Patch ~erc~ to use SASL using code snippets from [[https://libera.chat/guides/emacs-erc][here]] & ~erc-sasl~ from [[https://www.emacswiki.org/emacs/ErcSASL][here]]
#+begin_src emacs-lisp
(use-package erc-sasl
  :straight (erc-sasl :host gitlab :repo "psachin/erc-sasl")
  :demand t
  :config
...
Add modules: render smiley icons
#+begin_src emacs-lisp
(use-package erc
  :custom
